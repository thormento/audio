...
import { awardSession, dayKey } from './utils/gamification.js';
...
/**
 * Versões anteriores não guardavam minutos por atividade nem dias.
 * Na primeira execução desta versão, reconstrói esses totais a partir do
 * histórico existente (até HISTORY_LIMIT sessões), sem alterar XP/nível.
 */
async function backfillStats() {
  const progress = await storage.getProgress();
  if (progress.statsBackfilled) return;
  const history = await storage.getHistory();
  const hasStats = Object.values(progress.activityMs || {}).some((ms) => ms > 0) || Object.keys(progress.days || {}).length > 0;
  if (!hasStats && history.length) {
    history.forEach((entry) => {
      const day = dayKey(entry.startedAt || entry.date || Date.now());
      let sessionMs = 0;
      Object.keys(entry.activities || {}).forEach((key) => {
        const ms = Math.max(0, Number(entry.activities[key]) || 0);
        progress.activityMs[key] = (progress.activityMs[key] || 0) + ms;
        sessionMs += ms;
      });
      progress.days[day] = (progress.days[day] || 0) + sessionMs;
      if (!progress.firstDay || day < progress.firstDay) progress.firstDay = day;
    });
    log('Estatísticas reconstruídas a partir do histórico:', history.length, 'sessões');
  }
  progress.statsBackfilled = true;
  await storage.saveProgress(progress);
}

async function restoreSession(reason) {
  await storage.ensureInitialized();
  await backfillStats().catch((err) => warn('Falha ao reconstruir estatísticas', err));
...
async function startPageCreation(message) {
  pageCreationState = {
    active: true,
    quantity: message.quantity || 1,
    useFile: message.useFile !== false,
    created: 0,
    log: [],
    tabId: null
  };

  // Abre a página de criação de páginas do Facebook
  const tab = await chrome.tabs.create({
    url: 'https://www.facebook.com/pages/creation/',
    active: true
  });
  pageCreationState.tabId = tab.id;

  return {
    message: `Iniciando criação de ${message.quantity} página(s)...`,
    log: pageCreationState.log.join('\n')
  };
}

function stopPageCreation() {
  pageCreationState.active = false;
  return { message: 'Automação parada' };
}

function getPageCreationStatus() {
  return {
    message: `Páginas criadas: ${pageCreationState.created}/${pageCreationState.quantity}`,
    log: pageCreationState.log.join('\n')
  };
}
...
const handlers = {
  ...
  'START_PAGE_CREATION': async (message) => startPageCreation(message),
  'STOP_PAGE_CREATION': async () => stopPageCreation(),
  'GET_PAGE_CREATION_STATUS': async () => getPageCreationStatus()
};
